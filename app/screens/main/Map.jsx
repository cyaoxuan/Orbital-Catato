...
export default function Map() {
    ...
    const onChangeSearch = (query) => {
        ...
        let filteredCats = allCats.filter((item) => item.name.toLowerCase().includes(query.toLowerCase()));
    ...
    return (
        <KeyboardAvoidingView
            ...
            <MapView testID="map"
                ...
                onPress = {() => {Keyboard.dismiss(); setShowPredictions(false);}}
                onRegionChange = {() => {Keyboard.dismiss(); setShowPredictions(false);}}
                ...
                {!error[0] && (
                    allCats.filter((cat) => cat.lastSeenLocation).map((cat, index) => 
                        <RenderMarker key={cat.catID} cat={cat} navigation={navigation} />
                    )
                )}
            </MapView>
            <View style={{ position: 'absolute', top: 10, width: "100%", height: 250, alignItems: "center" }}>
            <Searchbar 
                placeholder="Search"
            ...
            {showPredictions && (<FlatList
                ...
                return (
                    <RenderPredictionRow item={item} onPress={() => animateToRegion(item.lastSeenLocation)}/>
                )
                }}
            />)}
        </View>
